...
import (
	...
	moviep "github.com/maestre3d/cinephilia/watch-list-service/internal/infrastructure/tracker/movie/persistence"
...
func main() {
	...
	movieRepo := moviep.NewPostgresMovieRepository(pqPool)
...
func execCrawl(ctx context.Context, commandBus domain.CommandBus) string {
	...
	err := commandBus.Dispatch(ctx, movieapp.CreateByCrawlCommand{
		MovieId:  movieId,
		UserId:   userId,
		CrawlUrl: "https://www.imdb.com/title/tt0469494/", // Uncut Gems by Safdie Brothers
	})
	if err != nil {
		log.Fatal(ddderr.GetDescription(err))
	}
